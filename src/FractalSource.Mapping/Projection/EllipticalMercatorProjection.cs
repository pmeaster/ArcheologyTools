...
{
    /// <summary>
    ///     This is the Mercator projection assuming the earth is a perfect ellipsoid.
    ...
    public class EllipticalMercatorProjection : GlobalMercatorProjection
    {
        ...
        public override double LatitudeToY(Angle latitude)
        {
            ...
            var sinOfPhi = Math.Sin(phi);

            var con = ReferenceGlobe.Eccentricity * sinOfPhi;
        ...
        public override double YToLatitude(double y)
        {
            ...
            var dOfPhi = 1.0;
            var i = 0;
            while ((Math.Abs(dOfPhi) > 0.000000001) && (i < 15))
            {
            ...
            var d = Angle.RadiansToDegrees(phi);
