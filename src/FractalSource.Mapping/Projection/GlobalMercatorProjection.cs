...
namespace FractalSource.Mapping.Projection
{
    public abstract class GlobalMercatorProjection : MercatorProjection
    {
        ...
        {
            var phi = Angle.DegreesToRadians(latitude);
        ...
        {
            return new EuclideanCoordinate(this)
            {
        ...
        {
            return new GeoCoordinates(YToLatitude(xy.Y), XToLongitude(xy.X));
        }
        ...
        public double[] GeoCoordinatesToXy(GeoCoordinates coordinates)
        {
            var e = ToEuclidean(coordinates);
            return new[] { e.X, e.Y };
        }
        ...
        public GeoCoordinates XyToGeoCoordinates(double x, double y)
        {
            return FromEuclidean(new EuclideanCoordinate(this, x, y));
        }
        ...
        /// <returns>The global coordinates</returns>
        public GeoCoordinates XyToGeoCoordinates(double[] xy)
        {
            var e = new EuclideanCoordinate(this, xy);
            return FromEuclidean(e);
        }
        ...
        {
            var longitude = ReferenceMeridian + Angle.RadiansToDegrees(x / ReferenceGlobe.SemiMajorAxis);
        ...
        public IEnumerable<GeoCoordinates> CalculatePath(
            ...
            int numberOfPoints = 10)
        {
            ...
            if (numberOfPoints < 2)
            {
                throw new ArgumentOutOfRangeException(nameof(numberOfPoints), "A geodetic path must have at least two points.");
            }
            ...
            var cStart = ToEuclidean(start);
            var cEnd = ToEuclidean(end);
            var dist = EuclideanDistance(cStart, cEnd);
            var step = dist / (numberOfPoints - 1);
            var dx = (cEnd.X - cStart.X) / dist;
            var dy = (cEnd.Y - cStart.Y) / dist;
            bearing = 0;
            bearing = Angle.RadiansToDegrees(Math.Atan2(dx, dy));
            ...
            if (bearing.IsApproximatelyEqualTo(90) || bearing.IsApproximatelyEqualTo(270))
            {
            ...
            else
            {
                ...
                mercatorRhumbDistance = ReferenceGlobe.SemiMajorAxis / Math.Cos(Angle.DegreesToRadians(bearing)) *
                                        (((1 - e2 / 4.0) * Angle.DegreesToRadians(end.Latitude - start.Latitude))
                                         -
            ...
            {
                var point = new EuclideanCoordinate(this, cStart.X + i * dx * step, cStart.Y + i * dy * step);
                result[i] = FromEuclidean(point);
            }
            return result;
        }
    }
