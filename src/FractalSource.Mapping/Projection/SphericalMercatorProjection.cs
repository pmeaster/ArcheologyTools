...
namespace FractalSource.Mapping.Projection
{
    ...
    public class SphericalMercatorProjection : GlobalMercatorProjection
    {
        ...
        {
            var latitude = Angle.RadiansToDegrees(2.0 * Math.Atan(Math.Exp(y / ReferenceGlobe.SemiMajorAxis)) - 0.5 * Math.PI);
        ...
        {
            return 
                ReferenceGlobe.SemiMajorAxis * Math.Log(Math.Tan(Math.PI / 4.0 + latitude.Radians / 2.0));
        }
    }
}